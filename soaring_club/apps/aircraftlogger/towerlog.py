...
from debug import debug_print #IGNORE:W0401
import debug #IGNORE:W0401
import csv
import glob
import math
import operator
...
class TowerLog(object):
    ...
    def handle_line(self, line):
        ...
        '''
        if len(line) <3 or not line.startswith('$') or '*' not in line:
        ...
        sentence_line, checksum = line[1:].strip().rsplit('*',1)
        ...
        try:
            ...
            debug_print('==>', dispatch, error.message, '\n\t\t\t', 
                        sentence, '\n\t\t\t', sentence_line)
    ...
    def process_position_time(self, 
        ...
        if status == "A":
            ...
            tz = pytz.timezone(TZ)
            utc_datetime = datetime.datetime(*(utcdate+utc))
            utc_datetime += datetime.datetime.utcoffset(tz.localize(utc_datetime))
            self.time = utc_datetime.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def process_plane_state(self, 
        ...
        if plane.is_in_unknown_position():
            if (float(speed) > self.takeoff_speed and 
                ...
                debug_print('==>Bad takeoff time:%s flarm_id:%s, speed:%s, altitude:%s' % 
